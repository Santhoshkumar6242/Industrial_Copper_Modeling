...
# Custom style for Prediction result text
...
# get input from user for both regression and classification method
...
class prediction():
    ...
    def regression():
        ...
        with st.form('Regression'):
            ...
            with col1:
                item_date = st.date_input(label='Item Date', min_value=date(2020,7,1), max_value=date(2021,5,31), value=date(2020,7,1))
                ...
                thickness_log = st.number_input(label='Thickness', min_value=0.1, max_value=2500000.0, value=0.1)

                product_ref = st.selectbox(label='Product Ref', options=options.product_ref_values)

            with col3:
                delivery_date = st.date_input(label='Delivery Date', min_value=date(2020,8,1), max_value=date(2022,2,28), value=date(2020,8,1))
        ...
        # user entered the all input value and click the button
        if button:
            
            # load the regression pickle model
            with open(r'C:\Users\Santhosh\Desktop\CapstoneProject\Capstone\Copper Industry\regression_model.pkl', 'rb') as f:
                model = pickle.load(f)
    ...
    def classification():
        ...
        if button:
            ...
            # load the classification pickel model
            with open(r'C:\Users\Santhosh\Desktop\CapstoneProject\Capstone\Copper Industry\classification_model.pkl', 'rb') as f:
                model = pickle.load(f)
...
with tab1:
    ...
    try:
        ...
        if selling_price:
            #apply custom css system for prediction text
            style_prediction()
        ...
        with col2:
            st.warning('### Quantuty Tons / Customer ID is empty')
...
with tab2:
    ...
    try:
        ...
        if status == 1:
            #appy custom css system for prediction text
            style_prediction()
            st.markdown(f'### <div class="center-text">Prediction Selling Price = Won</div>', unsafe_allow_html= True)
            st.balloons()

        if status == 0:
            #appy custom css system for prediction text
            style_prediction()
            st.markdown(f'### <div class="center-text">Prediction Selling Price = Lost</div>', unsafe_allow_html= True)
            st.snow()
        ...
        with col2:
            st.warning('### Quantuty Tons / Customer ID / Selling Price is empty')